...
class BaseBoard {
  public:
    ...
    BaseConfig *get_config() { return config; }
    ...
    std::shared_ptr<Display<T>> get_display(Args &&...args) {
        return std::make_shared<Display<T>>(std::forward<Args>(args)...,
                                            config->i2c_base_path);
    }
    ...
    std::shared_ptr<Pin> get_pin(int pin_number) {
        if (pins.find(pin_number) == pins.end()) {
            ...
            pins[pin_number] =
                std::make_shared<Pin>(pin_number, config->gpio_base_path);
        }
    ...
    std::shared_ptr<I2C> get_i2c(int i2c_number, int dev_address) {
        if (i2cs.find(i2c_number) == i2cs.end()) {
            ...
            i2cs[i2c_number] = std::make_shared<I2C>(i2c_number, dev_address,
                                                     config->i2c_base_path);
        }
