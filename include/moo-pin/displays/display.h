...
template <typename T> class Display {
  public:
    ...
    std::shared_ptr<T> get_underlaying_display() { return display; }
...
template <> class Display<moo::SSD1306> {
  public:
    Display(uint8_t lines, uint8_t columns, int i2c_number,
    ...
    void off() { display->onoff(0); }

    void set_cursor(int x, int y) { display->set_XY(x, y); }
    ...
    void write(std::string &message, int x = 0, int y = 0) {
        ...
        display->write_string(current_font,
                              const_cast<char *>(message.c_str()));
    }
    void clear() { display->clear_screen(); }

    void clear(int line) { display->clear_line(line); }
