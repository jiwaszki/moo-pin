...
class SSD1306 : public BaseDisplay {
  public:
    SSD1306(int i2c_number, const std::string &i2c_base_path) {
        display_i2c =
            std::make_shared<I2C>(i2c_number, SSD1306_I2C_ADDR, i2c_base_path);
        // Test the connection
    ...
    void default_config(uint8_t lines, uint8_t columns) {
        if (lines != SSD1306_128_64_LINES && lines != SSD1306_128_32_LINES &&
            SSD1306_64_48_LINES)
            lines = SSD1306_128_64_LINES;

        if (columns != SSD1306_128_64_COLUMNS &&
            lines != SSD1306_128_32_COLUMNS && SSD1306_64_48_COLUMNS)
        ...
        data_buf[i++] = SSD1306_COMM_HORIZ_NORM;  // SEGREMAP  Mirror screen
                                                 // horizontally (A0)
        data_buf[i++] =
    ...
    void write_line(uint8_t font_size, char *ptr) {
        ...
        while (ptr[index] != 0 && i <= 1024) {
            ...
            uint8_t *font_ptr =
                &font_table[(ptr[index] - 0x20) * font_table_width];
            uint8_t j = 0;
    ...
    void load_resolution() {
        ...
        max_columns =
            static_cast<uint8_t>(std::stoi(line.substr(0, delimiterPos)));
        max_lines =
            static_cast<uint8_t>(std::stoi(line.substr(delimiterPos + 1)));
    }
